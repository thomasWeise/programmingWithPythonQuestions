...
\qENDE{%
Show the syntax of classes in \python. Include and additionally explain the following elements\begin{enumerate}%
\item the class name,
...
Zeigen Sie die Syntax von Klassen in \python. Beinhalten und erklären Sie die folgenden Elemente\begin{enumerate}%
...
\qENDE{%
Name and explain two general use cases of classes.%
}{%
Nennen und erklären Sie zwei generelle Szenarien, in denen man Klassen verwenden kann/sollte.%
}%
\end{question}%
%
%
\begin{question}{What is it? / Was ist das?}%
\qENDE{%
Explain what \emph{immutable} class (instances) are:\begin{enumerate}%
...
Erklären Sie, was \emph{unveränderliche} Klassen(-Instanzen) sind:\begin{enumerate}
...
\qENDE{%
In which situation would we start the name of an attribute of a class with two underscore characters, e.g., \pythonil{__counter}?%
}{%
In welcher Situation würden wir den Name eines Attributs einer Klasse mit zwei Unterstrichen beginnen, zum Beispiel~\pythonil{__counter}?%
}%
\end{question}%
%
%
...
\qENDE{%
The output of program \programUrl{class_01} is given in \cref{exec:class_01} above. %
...
Die Ausgabe des Programms \programUrl{class_01} ist oben in \cref{exec:class_01} gegeben. %
...
\end{align}%
\qENDE{%
The above equations demonstrate mathematical operations for one to two complex numbers~$z_1,z_2\in\complexNumbers$ such that $z_1=a_1+b_1\imaginaryUnit$ and $z_2=a_2+b_2\imaginaryUnit$, where \imaginaryUnit\ is the imaginary unit with~$\imaginaryUnit^2=-1$~\cite{VDC2022AITCN}.\begin{enumerate}%
...
Die Gleichungen oben stellen Operationen für ein bis zwei komplexe Zahlen~$z_1,z_2\in\complexNumbers$ mit $z_1=a_1+b_1\imaginaryUnit$ und $z_2=a_2+b_2\imaginaryUnit$ dar, wobei \imaginaryUnit\ die imaginäre Einheit mit~$\imaginaryUnit^2=-1$ ist~\cite{VDC2022AITCN}.\begin{enumerate}%
...
\end{align}%
\qENDE{%
The above equations demonstrate mathematical operations for two rational numbers~$q_1,q_2\in\rationalNumbers$ such that $q_1=\frac{a_1}{b_1}$ and $q_2=\frac{a_2}{b_2}$.
...
Die Gleichungen oben demonstrieren die mathematischen Operationen für zwei rationale Zahlen~$q_1,q_2\in\rationalNumbers$ mit $q_1=\frac{a_1}{b_1}$ und $q_2=\frac{a_2}{b_2}$.
...
\begin{question}{Programming / Programmieren}%
\qENDE{%
Let's build a class hierarchy for life forms. Write code for everything below.%
\begin{enumerate}%
...
This subclass gets a method \pythonil{do_something(self, hungry: bool)}, which invokes the \pythonil{eat} method if \pythonil{hungry == True}.%
\end{enumerate}%
}{%
Lassen Sie uns eine Klassenhierarchie für Lebensformen bauen. Schreiben Sie Kode für \emph{alles} Folgendes:%
\begin{enumerate}%
...
Diese Subklasse bekommt eine Methode \pythonil{do_something(self, hungry: bool)}, die die \pythonil{eat}-Methode aufruft wenn \pythonil{hungry == True}.%
\end{enumerate}%
}%
\end{question}%
%
